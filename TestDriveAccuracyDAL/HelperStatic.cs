...
namespace TestDriveAccuracyDAL
{
    public class HelperStatic
    {
        ...
        /// <param name="p1"></param>
        /// <param name="p2"></param>
        /// <returns>TODO here: return null and not ""</returns>
        ...
        /// <summary>
        /// Directory check: if Validroot? Does dit exist? 
        /// </summary>
        /// <param name="theFileName">Full directory name</param>
        /// <returns>bool</returns>
        public static bool DirChecks(string theDirPath)
        {
            //Check if path sting contains valid root else false
            if (!Path.IsPathRooted(theDirPath))
                return false;
            //if the directory not there return false
        ...
        /// </summary>
        /// <param name="FullDataFilePath"></param>
        /// <returns></returns>
        public static string DirectoryFromFilepath(string FullDataFilePath)
        {
        ...
        public static StringBuilder readAdditionalInfo(string fullDataFilePath)
        {
            ...
            //checks if the fiel is open or in use
            if (!FileChecks(fullDataFilePath))
                return uutdetail.Append("File for additional info does not exist: error");

            //Need FileInfo 
            FileInfo FI = new FileInfo(fullDataFilePath);
            if (IsFileLocked(FI))
            {
                uutdetail.Append("The file with general test info is open or does not exist! error");
                return uutdetail;
            }
            ...
            if (!Path.IsPathRooted(fullDataFilePath))
                return uutdetail.Append("Path string does not contain valid root : error");
            //Create dir name from full filename
            string theDirPath = Path.GetDirectoryName(fullDataFilePath);
            //if the directory not there return a message
            if (!Directory.Exists(theDirPath))
                return uutdetail.Append("The Directory does not exits: error");
            ...
            {
                uutdetail.Append("The .txt info file created by sequencer is not in the results folder: error");
            }
            return uutdetail;
        }
        /// <summary>
